...
- **Synchronous and reactive HTTP clients** — supports both `RestTemplate` (JDK HttpClient, Apache HttpClient 5) and `WebClient` (Reactor Netty), selectable via a single `client-type` property (`jdk`, `apache`, or `netty`). Aliases `rest`, `servlet`, and `reactive` are also accepted.
...
- `RestTemplateUtil` — synchronous retry (`executeWithRetry`), error handling (`handleError`), `emptyOn404()`, `emptyOn(HttpStatus...)`.
...
### [1.0.9]
- Added Mutual TLS protocol support.
...
### [1.0.5]
- Removed configuration property `cacheName` from `OpenidTokenProperties`.
- Fixed providers' local caching issue when multiple connections are configured.
- Added `opentmf-basic-webclients-starter` that dynamically exposes beans from configuration.
- Added `opentmf-openid-webclients-starter` that dynamically exposes beans from configuration.

### [1.0.4]
- Added configuration property `usernameField` to `OpenidTokenProperties`.
...
### [1.0.1]
- Fixed the `BasicWebClientProviderAutoConfiguration` class name.
