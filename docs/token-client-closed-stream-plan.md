...
**Status:** IMPLEMENTED in 2.2.0 (PR #1) — kept as the record of why · **Target:** 2.2.0 (see §6 for why MINOR) · **Origin:** e2e finding row 13,
...
Kept under `docs/` on request for this cut (the usual delete-after-implementation convention is
suspended); the operational facts also live in the README's token-fetch section and the 2.2.0
CHANGELOG entry. Two things learned while implementing, beyond §2: WebFlux surfaces a body-stage
I/O failure as a `WebClientResponseException` **with status 200** ("… but response failed with
cause: IOException"), so the reactive predicate checks `!status.isError() && hasIoCause` rather
than excluding every response exception; and MockServer's
`error().withResponseBytes(<status line + headers>).withDropConnection(true)` reproduces the
finding's exact chain (`RestClientException → IOException("closed") → EOFException`).
